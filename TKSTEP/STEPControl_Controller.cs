...
namespace TKSTEP
{
    ...
    public class STEPControl_Controller : XSControl_Controller
    {
        ...
        const string OCC_VERSION_STRING = "7.7";
        public STEPControl_Controller() : base("STEP", "step")
        {
            if (!init)
            {
                ...
                //ExceptionRegion.  Standard_STATIC_ASSERT((int)Resource_FormatType_CP850 - (int)Resource_FormatType_CP1250 == 18); // "Error: Invalid Codepage Enumeration"

                init = true;
            }

            //STEPControl_ActorWrite ActWrite = new STEPControl_ActorWrite;
            //ActWrite->SetGroupMode(Interface_Static::IVal("write.step.assembly"));
            //myAdaptorWrite = ActWrite;

            StepSelect_WorkLibrary swl = new StepSelect_WorkLibrary();
            swl.SetDumpLabel(1);
            ...
            //IFSelect_Selection xmr= SessionItem("xst-model-roots");
            //if (!xmr.IsNull())
            //{
            ...
            ////pdn
            //STEPSelections_SelectFaces stfaces = new STEPSelections_SelectFaces();
            ...
            //STEPEdit_EditSDR edsdr = new STEPEdit_EditSDR();
            //IFSelect_EditForm edsdrf = new IFSelect_EditForm(edsdr, false, true, "STEP Product Data (SDR)");
    ...
    public class RWStepAP214
    {
        static int THE_RWStepAP214_init = 0;

        public static void Init()
        {
            if (THE_RWStepAP214_init != 0)
            {
                return;
            }
            THE_RWStepAP214_init = 1;
            RWHeaderSection.Init();
            StepAP214_Protocol proto = StepAP214.Protocol();
    ...
    public class RWStepAP214_GeneralModule : StepData_GeneralModule
    {
    }
